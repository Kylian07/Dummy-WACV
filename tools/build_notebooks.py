...
def driver_cells(dataset_key: str, info: Dict) -> List[Dict]:
    ...
    return [
        ...
        md("### 5.3 Smoke test (~2 minutes)\n\n"
           ...
           "> ### This is a plumbing check. Its numbers are not results.\n"
           ">\n"
           "> At 2 epochs the dictionary has barely trained, and the causal metrics "
           "then track **dictionary quality rather than state structure**. The "
           "failure is systematic, not random: when the dictionary compresses the "
           "sketch worse than the raw evidence does, ablating code content pushes "
           "`S` back toward `g(x)` and *improves* the mask — so necessity and CSI "
           "go **negative**, and the reasoning loop looks like a liability, for "
           "reasons that have nothing to do with sparsity.\n"
           ">\n"
           "> The readiness block printed below is what tells you this has "
           "happened. Do not read T2 until its hard checks pass."),
        code(
            ...
            "\n"
            "# Expect hard checks to FAIL here. That is the smoke test working: it is\n"
            "# telling you 2 epochs is not a trained model, which is exactly the\n"
            "# judgement you would otherwise have to make by eye from a confident-\n"
            "# looking but meaningless causal table.\n"
            "print_readiness(_smoke)"
        ),
        md("### 5.4 The real run\n\n"
        ...
        md("### 5.7 What to check before believing the result\n\n"
           "`print_report` now runs these automatically and **gates T2 on them** — "
           "this list is the reasoning behind that gate.\n\n"
